...
k_mets = kfolds_cv(10, cleaned_data, run_boost=T)
k_mets
```
...
library(caret)
library(xgboost)
test.x.mat = as.matrix(cleaned_data[,-which(names(cleaned_data) %in% c("POSTSEASON"))])
boost.model.final = train(POSTSEASON~., data=cleaned_data, method="xgbTree")
pred.boost.final = boost.model.final %>% predict(as.data.frame(test.x.mat))
output = cbind(cleaned_data.team[, which(names(cleaned_data.team) %in% c("YEAR", "TEAM", "POSTSEASON"))], pred.boost.final)
write.csv(output[output$YEAR == "2019",], "boost_predictions.csv", row.names = F)
summary(boost.model.final)
varImp(boost.model.final)
```
